...
class Population:
    ...
    def fitness(self):
        for deme in self.subpops:
            deme.fitness = 1 + (deme.nind[1]/self.nind)*self.selcoeff*self.gencoef + (deme.nind[2]/self.nind)*self.selcoeff
    ...
    def migrate(self):
        self.totgam[0] = np.sum([deme.gam[0]*deme.fitness for deme in self.subpops])
        self.totgam[1] = np.sum([deme.gam[1]*deme.fitness for deme in self.subpops])
        self.total_fitness = np.sum([deme.fitness for deme in self.subpops])

        for deme in self.subpops:
            deme.migs[0] = (self.totgam[0] - deme.gam[0]*deme.fitness)/(self.total_fitness - deme.fitness)
            deme.migs[1] = (self.totgam[1] - deme.gam[1]*deme.fitness)/(self.total_fitness - deme.fitness)
    ...
    def next_generation(self):
            ...
            self.migrate()
            self.fuse()
            if(self.extincitonrate > 0): self.extinct()
